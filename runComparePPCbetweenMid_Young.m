...
% given protocol
function runComparePPCbetweenMid_Young()
 
    % Create the GUI
    fig = figure('Name', 'PPC comparision between mid aged and young subjects', 'NumberTitle', 'off', ...
    ...
    uicontrol('Parent', controlPanel, 'Style', 'text', 'Position', [1100, 20, 190, 20], 'String', 'Freq Range (Hz):');
...
% plot handles _______________________________________
    ...
    function plotConnectivity()
        ...
        midSubjectNameList = subjectNameList(ageListAllSub>=40);
            ...
            subjectNameListsM{1} = intersect(subjectNameLists{1},midSubjectNameList,'stable'); % mid aged meditators
            subjectNameListsM{2} = intersect(subjectNameLists{2},midSubjectNameList,'stable'); % men controls
  
            subjectNameListsY{1} = intersect(subjectNameLists{1},youngSubjectNameList,'stable'); % young med
            subjectNameListsY{2} = intersect(subjectNameLists{2},youngSubjectNameList,'stable');
...
[connDataM,connDataY, freqVals,connDataElectrodeGroupM,connDataElectrodeGroupY, electrodeGroupList,groupNameList,binnedCenters] = getConnDataAllSubjects(subjectNameListsM,subjectNameListsY, refElectrodes,groupType,connMethod,badEyeCondition,badTrialVersion,protocolName, analysisChoice,cutoffList,pairedDataFlag,saveFolderName,capType);
...
for m=1:numElectrodeGroups
    ...
    dataToPlot{2} = squeeze(connDataElectrodeGroupY{1}(:,m,:)); % med , Bl
    else
    dataToPlot{1} = squeeze(connDataElectrodeGroupM{2}(:,m,:)); % con, st
    dataToPlot{2} = squeeze(connDataElectrodeGroupY{2}(:,m,:)); % con , bl
    end
    ...
    for j=1:3 % 
        ...
        %x = squeeze(mean(connDataElectrodeGroupSt{j}(:,:,freqPosList{i}),3));
          if strcmp(subjectsChoice, 'med')
            ...
            dataToCompare{2} = squeeze(mean(connDataElectrodeGroupY{1}(:,:,freqPosList{i}),3));
          else
            dataToCompare{1} = squeeze(mean(connDataElectrodeGroupM{2}(:,:,freqPosList{i}),3));
            dataToCompare{2} = squeeze(mean(connDataElectrodeGroupY{2}(:,:,freqPosList{i}),3));
          end
...
% __________________________________________________________________________
 function [connDataM,connDataY, freqVals,connDataElectrodeGroupM,connDataElectrodeGroupY, electrodeGroupList,groupNameList,binnedCenters] = getConnDataAllSubjects(subjectNameListsM,subjectNameListsY, refElectrodes,groupType,connMethod,badEyeCondition,badTrialVersion,protocolName, analysisChoice,cutoffList,pairedDataFlag,saveFolderName,capType)
...
for i=1:2 % med and con
    for j=1:length(subjectNameListsY{i}) % no. of subjects in 'i'
        subjectNameM = subjectNameListsM{i}{j};
        subjectNameW = subjectNameListsY{i}{j};
...
connDataElectrodeGroupY = cell(1,3);
...
numSubjects = length(x1);
    ...
    for p=1:numSubjects
        ...
        connDataElectrodeGroupY{q}(p,:,:) = y2{p};
    end
end
